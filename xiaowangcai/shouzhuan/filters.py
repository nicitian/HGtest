        ...
        if get_in_request(request, 'store_id'):
            ...
            if tid:
                if Task.objects.get(pk=tid).store_id != sid:
                    return False
            if get_store_user_id(sid) == s_uid and Store.objects.get(pk=sid).verify_status == Const[
                'model.verify.check_pass']:
                return True
    return False
...
def app_check(f):
    ...
    def inner(request, *args, **kwarg):
        ...
        if 'Android' in request.META.get('HTTP_X_REQUEST_CLIENT') and request.c_versioncode < 78:
