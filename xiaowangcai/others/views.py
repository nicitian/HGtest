        ...
        for x in xrange(1, 101):
            ...
            for g in goods:
                ...
                try:
                    ...
                    import datetime,urllib,md5
                    sign=""
                    appsecret="215b488a196fa48ef858a537cd308ed5"
                    appkey = "23741851"
                    #获取当前时间
                    timestamp=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    url = tkgoods.quan_goods_link
                    text = tkgoods.title
                    para = "{'url': '" + url + "', 'text': '"+ text +"'}"
                    parame=[{'app_key': appkey},
                            {'tpwd_param': str(para)},
                            {'format':'json'},
                    ...
                    parame.sort() #按照a-z字符顺序排列
                    #字符串拼接
                    for item in parame:
                        sign+=item.keys()[0]+item.values()[0]
                    ...
                    for item in parame:
                        apiurl+='&' + item.keys()[0]+ '=' + urllib.quote(item.values()[0])
                    # print apiurl
                    import requests
                    ...
                    try:
                        print rr['wireless_share_tpwd_create_response']['model']
                        tkgoods.tkl = rr['wireless_share_tpwd_create_response']['model']
                    except Exception as e:
                        print e
                        continue
                tkgoods.fr = 2
