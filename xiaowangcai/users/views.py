...
from shouzhuan.utils import JsonResponse, msec_time, get_paginator, fix_length_random_int, make_conditions, sub_dict, \
    obj_to_dict, \
...
def smscaptcha_request(r, phone):
    ...
    try:
        ...
        s = Smscaptcha.objects.create(phone=phone, captcha=capt, create_time=nowtime)
    result = SMSService.sendTemplateSMS(phone, [capt, Const['sms.valid.time']], 133993)
...
def tbaccount_list(r):
    start, num = get_paginator(r)
    condition = make_conditions(r, user_id='user_id', wangwang='tb', status='verify_status')
    if 'register_start_time' in r.REQUEST:
...
def refresh_randcode(r):
    ...
    def create_lines():
        ...
        for i in range(line_num):
            ...
            draw.line([begin, end], width=2, fill=fg_color)
    ...
    def create_strs():
        """绘制验证码字符"""
        c_chars = random.sample(init_chars, length)
        strs = ' %s ' % ' '.join(c_chars)  # 每个字符前后以空格隔开

        font = ImageFont.truetype(font_type, font_size)
        font_width, font_height = font.getsize(strs)  # 66 16
    ...
    r.session['randcode'] = strs.lower()
    import base64
    return JsonResponse(code=Const['code.success'], data={'img': base64.b64encode(mstream.getvalue())})
    # return HttpResponse(base64.b64encode(mstream.getvalue()), "image/gif")
